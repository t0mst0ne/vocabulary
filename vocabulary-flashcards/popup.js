document.addEventListener('DOMContentLoaded', () => {
  ...
  function generateQuestion() {
    try {
      ...
      options.forEach(opt => {
        ...
        // Truncate removed to show full answer
        // if (defText.length > 80) defText = defText.substring(0, 77) + '...';
  ...
  function extractEnglish(text) {
    if (!text) return "";
    // Format is usually "Chinese (English)"
    // Regex to capture text inside last parentheses
    const match = text.match(/\(([^)]+)\)$/);
    if (match && match[1]) {
      return match[1];
    }
    // If no parentheses, check if it contains Chinese characters
    // If it has Chinese, try to return the part that looks like English?
    // Or just return as is if we can't parse.
    return text;
  }
  ...
  function saveMistakes() {
    if (chrome.storage) {
      chrome.storage.local.set({ 'vocabulary_mistakes': mistakes });
    }
  }

  function loadMistakes() {
    if (chrome.storage) {
      chrome.storage.local.get(['vocabulary_mistakes'], (result) => {
        if (result.vocabulary_mistakes) {
          mistakes = result.vocabulary_mistakes;
          updateDownloadBtn();
        }
      });
    }
  ...
  async function fetchMerriamWebsterData(url) {
    ...
    try {
      ...
      // MW content is usually in div[id^="dictionary-entry-"]
      const entries = doc.querySelectorAll('div[id^="dictionary-entry-"]');

      if (entries.length > 0) {
        mwContainer.innerHTML = '<h3>Merriam-Webster Source:</h3>';

        // Process each entry (usually just the first one or two is enough)
        // Let's take the first one for now to keep it concise
        ...
        links.forEach(a => {
          ...
          a.target = '_blank';
          // Remove styles to blend in, or keep them if they look okay
          // For now, let's strip non-essential links like we did for Cambridge?
          // MW links are usually cross-references. Let's keep them but style them.
      ...
      mwContainer.innerHTML = '<p>Failed to load Merriam-Webster content.</p>';
    }
  }
  ...
  async function fetchCambridgeData(url) {
    ...
    // Create container for Cambridge data if not exists
    ...
    try {
      ...
      // Try to find the main definition block
      // Strategy: Look for .di-body (main entry) or .def-block (specific definition)
      const entryBody = doc.querySelector('.di-body') || doc.querySelector('.def-block');

      if (entryBody) {
        // Clean up: Remove scripts, styles, and unnecessary elements
        const scripts = entryBody.querySelectorAll('script, style, .runon-title, .runon-body');
        scripts.forEach(el => el.remove());

        // Extract relevant parts (Simplified)
        // We want to show the first few definitions and examples
        // Let's try to just take the whole entryBody but limit height/style

        // Fix relative links and remove non-idiom links
        const links = entryBody.querySelectorAll('a');
        links.forEach(a => {
          // Check if it's an idiom link
          // Idiom links usually have title ending in 'idiom', 'phrasal verb', or start with 'meaning of'
          // or contain .idiom class
          ...
          if (isIdiom) {
            ...
            a.target = '_blank';
            a.style.textDecoration = 'underline'; // Keep underline for idioms
            a.style.color = '#1d4ed8';
          } else {
            // It's a normal word link (cross-reference), remove the link but keep text
            // We can replace with a span or just strip the href and style
            const span = document.createElement('span');
            span.innerHTML = a.innerHTML;
            span.className = 'stripped-link';
            // Copy relevant classes if needed, but usually we want it to look like plain text
      ...
      cambridgeContainer.innerHTML = '<p>Failed to load Cambridge content (likely blocked or network error).</p>';
    }
  }
  ...
  function renderAnalysis(data) {
    ...
    if (most_common_meaning) {
      ...
      section.appendChild(createDetailItem('例句', most_common_meaning.example)); // Note: No Chinese translation for example available in offline mode
